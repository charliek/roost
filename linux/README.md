...
Linux-only crate here — the UI code lives in `crates/roost-iced/`
(iced, the packaged UI); this directory only builds and stages the
package.
...
production `linux` bundle profile (same socket, `state.json`, and log
dir the pre-iced GTK package used to own), so the package is a clean
upgrade in place for existing users. See
