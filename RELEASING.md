...
| Tag pushed, `version-check` fails | Tagged a commit that didn't run `update-version.sh` | Re-bump locally, then re-tag — see the next row for whether the tag may move |
| Tag pushed, the run failed before `publish-release` | `ci-gate` refused a red commit, or a build job failed | Nothing is external yet: `create-release` only makes a draft. Fix forward on `main`, wait for CI, then move the tag: `git push origin :refs/tags/vX.Y.Z`, re-tag at the fix commit, push the tag. **Once `publish-release` has run, never move the tag** — cut a fresh patch version instead. v0.0.20 moved its tag twice this way |
