...
pub fn advance(data: &[u8], xmtx: u32, long_metric_count: u16, glyph_id: u16) -> u16 {
    // XXX: roost delta vs upstream 0.2.10 (issue #292, see README.roost.md):
    // a font declaring zero long metrics underflows the else branch below in
    // debug builds. Returning 0 matches release semantics, where the wrapped
    // offset's out-of-bounds read is absorbed by unwrap_or(0).
    if long_metric_count == 0 {
        return 0;
    }
