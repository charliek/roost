...
- `busy` is the one server code worth retrying: it means a local-backend switch is in flight. An older Roost without `busy` refuses the same switch as `host-unavailable` instead; when `identify --json` still shows `local_backend_switch`, treat that the same as `busy`. `roostctl open` refuses a switch in flight as `usage` (exit 2, "a backend switch is in progress; retry") — that is the same retryable condition too. `wait`, `tab prompt`, and `events` already hold off on both refusals themselves, so this retry rule is for every other verb: poll `identify --json` until `local_backend_switch` is absent, then retry, re-reading any tab or project id first.
