...
- `detail` — the long form behind `reason`, when there is one the band has no room for. Absent today; the field is in the shape so callers can read it when the localhost spawn ladder starts filling it.
