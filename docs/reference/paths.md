...
| `ROOST_SESSION_NO_SEED` | Only the spawn a [local-backend switch](../guides/host-sessions.md#switching-the-local-backend)'s forward direction performs, for its own destination | Tells a first-ever session **not** to seed itself a project. Set because that spawn is immediately followed by the switch's own replay — a session that seeded itself first would hand you a stray empty project sitting beside the layout you just moved over. Every other spawn gets the ordinary seeded behavior. |
