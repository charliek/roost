# `roost-cli` (Go prototype)

!!! info "Legacy section"
    This describes the Go companion CLI that ships from `main`. The Rust-binary replacement is `roost-cli-rs` — see [Reference → CLI](../cli.md). The Rust CLI renames to canonical `roost-cli` in Phase 9.
