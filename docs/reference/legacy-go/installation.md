...
!!! info "Legacy section"
    This describes the Go + GTK4 binary that ships from `main`. For the Rust core + native UI path, see [Getting Started → Installation](../../getting-started/installation.md).
...
Roost falls back to Monaco if JetBrains Mono isn't installed, but the fallback path on macOS is finicky (Pango can drop to Verdana when the requested family is missing) — see [Config keys](../paths.md#config-keys) to override the family or set up your own preference order.
...
`roost-cli identify` should print a JSON object containing the socket path and the active project / tab IDs. If you see a connection error, the GUI isn't running or the socket path is wrong — see [Paths & Environment](../paths.md).
