...
!!! info "Legacy section"
    This describes iterating on the Go module that ships from `main`. The Rust + Swift + Linux Rust development workflow is at [Development → Setup](../../development/setup.md).
...
See [Installation](installation.md) for the per-platform package commands.
...
If the bump also moves past Zig 0.15.2, drop the `maybe_arm64_sdk_shim` helper in `build/build.sh` — it exists only because Zig 0.15.2 links host artifacts as `arm64-macos`, which Apple's macOS 26+ SDK no longer exposes. See the macOS 26 section of [Installation](installation.md).
...
- All UI calls happen on the main thread; goroutines marshal via `glib.IdleAdd`. See [Architecture](architecture.md#threading-contract).
