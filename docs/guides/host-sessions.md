...
- **The session-wide settings — theme and agent-hook mode — are last-writer-wins**, from whichever client sent them most recently. Two clients that disagree flip them on every reconnect; that is by design, not a bug (see [Agent Hooks](agents.md#remote-hosts) for the file side of it). **File uploads are not a setting and do not compete**: every upload lands in a directory of its own, named at random, so two clients sending the same filename at the same moment get two files rather than one overwriting the other.
