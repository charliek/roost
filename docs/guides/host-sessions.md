...
| No session on the far end | *"`<target>` is reachable but has no roost session running. Run `roostctl session start` on that machine, then try again."* | Exactly what it says — SSH itself is fine, there's just nothing to attach to yet. |
| `roost-session` not found | *"roost-session isn't installed on `<target>` (or isn't on the non-interactive PATH ssh uses there)."* | **The #1 gotcha in practice.** Roost execs the remote command as `sh -c '... exec roost-session client-bridge'` — a non-interactive, non-login shell, so anything a login-only rc file (`.bash_profile`, `.zprofile`) or an interactive-only guard adds to `PATH` is invisible to it. Roost tries `$HOME/.local/bin/roost-session` first and falls back to a bare `roost-session` resolved off that non-interactive `PATH` — installing it to `~/.local/bin` sidesteps the whole PATH question. |
