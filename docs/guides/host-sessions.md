...
**A host tab's own attention doesn't reach you (older sessions only).** Roost tells a host session what you are actually looking at — which tab is selected, and whether the window has focus — so the same-tab suppression rule ([Notifications → Focus policy](notifications.md#focus-policy)) applies to the tab you are really watching, exactly as it does locally; if another window or a phone is also connected, a tab is muted while any of them is watching it (see [Several windows on one session](#several-windows-on-one-session)). This needs a session speaking the current protocol. An older `roost-session` does not reach a connected state at all — the compatibility gate refuses it and offers the update (over SSH) or the restart (on `localhost`), so there is no version of this where you are connected and merely missing the focus reporting.
