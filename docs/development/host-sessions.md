...
The cost of that third rung is recorded on `RestartAction::OfferRemoteUpdate` where it bites: the remote-update offer is raised only out of `NeedsRestart`, so a skewed **ssh** host now connects instead of reaching the one state that could have offered to update it. There is no on-demand replacement yet — an action that does not need a terminal connection state to hang off is follow-up work ([#447](https://github.com/charliek/roost/issues/447)) — so the manual route in the [guide](../guides/host-sessions.md#when-a-build-skew-connects-instead-the-vt-fallback) is the whole answer today.
