...
//! `session.identify` and `session.stop` start answering, data
//! connections are served, tab sizes default to this session's stated
//! geometry rather than a window's, and every mutating op becomes
...
//! * [`agent_hooks`] — what `session.set_agent_hooks` does: the
//!   connected client's `agent-hooks` config, applied to this host's
//!   dotfiles.
