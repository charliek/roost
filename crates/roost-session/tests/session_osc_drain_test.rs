//! OSC through the server terminal, end to end from a real child's
//! bytes.
//!
//! A tab's title and directory arrive as OSC sequences *inside* its
//! output stream, so somebody has to be reading that stream or they
//! freeze at whatever they were when the tab opened. On a session that
//! somebody is the tab task, which scans every chunk on its way into the
//! authoritative terminal. A scripted shell emits the sequences and
//! `tab.list` is asked what the workspace made of them — no internal
//! state is inspected, because the whole point is that a remote client
//! sees the result.
...
/// `\033` versus `\0033`, and a test of the *scan* should not be able to
