...
/// A lease-gated op with no side effect worth caring about — a null
/// focus claims nothing — used here purely to read what the gate
/// answers. What it does to the focus is `session_focus_test`'s
/// business.
async fn set_null_focus(f: &Fixture, c: &Conn, lease: &str) -> Result<(), String> {
    let params = serde_json::json!({
        "lease": lease,
        "focused_tab_id": serde_json::Value::Null,
    });
    match f
        .handler
...
async fn takeover_replaces_the_lease_and_tombstones_the_old_one() {
    ...
    assert_eq!(
        set_null_focus(&f, &conn(3), &old).await.unwrap_err(),
        "taken-over"
    );
    set_null_focus(&f, &second, &new)
        .await
