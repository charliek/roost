...
/// [`DrawCell`] and [`RenderedRow`] are pure dump-densification built
/// entirely on `roost-vt` types, so they live in `roost-vt` (`src/resolve.rs`)
/// and are re-exported here so every call site in this crate keeps working
/// unchanged.
pub use roost_vt::{DrawCell, RenderedRow};
