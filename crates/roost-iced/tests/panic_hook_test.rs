...
fn run_forced_panic(variant: &str) -> (TempDir, Outcome) {
    ...
    let mut command = Command::new(env!("CARGO_BIN_EXE_roost-iced"));
    ...
    let mut child = command.spawn().expect("spawn roost-iced");
    ...
    let status = loop {
        match child.try_wait().expect("poll child") {
            ...
            None if started.elapsed() >= EXIT_TIMEOUT => {
                ...
                panic!(
                    "roost-iced did not exit within {EXIT_TIMEOUT:?} for ROOST_TEST_PANIC={variant}\n{}",
