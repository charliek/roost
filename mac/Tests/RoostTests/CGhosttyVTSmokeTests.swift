...
func libghosttyVtRoundTrip() {
    ...
    // GHOSTTY_SUCCESS is 0 by C convention; bindgen's Rust side and the
    // Swift importer both agree on this. If libghostty-vt ever changes
    // this value (it won't — it's part of the API contract) we'd catch
    // it here.
    #expect(rc == 0, "ghostty_terminal_new should succeed (got rc=\(rc))")
