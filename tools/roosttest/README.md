...
make e2e-host-localhost       # the `localhost` sentinel itself: its ↻ Restart verb and R15's takeback (#460)
...
| `test_session_attach.py` | The attach **data plane** against a real daemon (plan 036, plan 053, plan 057): a takeover that moves the foreground and closes nothing, many clients on one tab all reading and typing, geometry following the last one that interacted, refusals that are one JSON line and a close, the kind negotiation (GHOSTSNP, `vt`, the build-skew fallback), the READY fence and the seq contiguity behind it, resume, the lifecycle endings (child exit / disconnect / stop / a SIGKILLed daemon), the headless server terminal, and the plan 036 D9 perf budgets. Speaks the wire through `dataplane.py` and decodes no payload — see that row. `session_daemon`-marked, so it rides `make e2e-session` beside the module above. |
| `test_session_effects.py` | What the daemon gained so an attached client can be a faithful terminal without owning one (plan 037 §3.6 + §3.7, plan 038 C6, plan 057): `tab.effect` fan-out for bells and OSC 52 writes plus the 256 KiB cap and the observer/driver split, `session.set_theme` reaching the colors the terminal answers its own queries with, `session.set_focus` moving which tab a session mutes and forgetting a focus whose client is gone, that a write into a session tab needs no lease (over IPC and through the real `roostctl tab send`), and the `ROOST_SESSION_FAKE_BUILD` seam behind the build-mismatch refusal. `session_daemon`-marked. |
| `test_host_client.py` | HS-2's **client** side (plan 037): a real `roost-session` daemon beside the harness UI, wired together with `host.add` + `host.connect` and observed entirely over IPC. Covers the roadmap acceptance (a marker survives disconnect + reconnect), attach fidelity (the session's `tab.dump_resolved` vs the UI's for the `h<host>.<id>` spelling), never-blank/no-duplication on refocus, disconnect-vs-stop, the build-mismatch `needs-restart` state and the restart composition, takeover (a scripted wire client takes the foreground; the frame stays **live** and typeable, the band names the taker, and `host.connect` takes the foreground back in place — plan 057, R15), foreground-only effects, the local/host id collision, the attach latency budget, and the attention-surface regressions. **Needs a UI *and* a daemon**, so it is in neither `ICED_E2E_TESTS` nor `SESSION_E2E_TESTS`: `make e2e-host-client` / `make e2e-host-client-ci`, its own Linux X11 + Wayland steps in `iced-build-e2e`, and the `host_client` marker every whole-directory run and every macOS iced cell deselects. |
