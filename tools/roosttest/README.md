...
| `fixtures/fake-ssh.sh` | The stand-in `ssh` both HS-3 lanes and `crates/roost-ipc/tests/ssh_transport_test.rs` drive: an argv log, the mux/`-O exit` choreography, and one failure mode per line of the classifier's table. Its header is the contract. |
